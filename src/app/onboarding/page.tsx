        ...
        {authState !== "authed" ? (
          <motion.div initial={{ y: 12, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.6, ease: "easeOut" }} className="w-full max-w-xl text-center">
            <h1 className="mb-10 text-5xl font-light uppercase tracking-wide text-neutral-100 md:text-6xl" style={{ fontFamily: 'SentinelBlack, ui-sans-serif, system-ui' }}>
              Edit images with AI. <span style={{ fontStyle: 'italic' }}>Now</span>.
            </h1>
          ...
          <div className="mt-8 space-y-8">
            <section>
              <div className="text-sm text-zinc-600">What would you like to do with your image?</div>
              <div className="mt-3 flex gap-3">
                <button onClick={() => setMode("scratch")} className={`rounded-md border px-4 py-2 ${mode === "scratch" ? "border-zinc-900" : ""}`}>Enhance & Style</button>
                <button onClick={() => setMode("improve")} className={`rounded-md border px-4 py-2 ${mode === "improve" ? "border-zinc-900" : ""}`}>Edit & Transform</button>
              </div>
            </section>

            <section>
              <div className="text-sm text-zinc-600">Upload your image and describe what you want</div>
              <div className="mt-3 grid gap-3">
                <input type="file" accept="image/*" onChange={(e) => setFiles(Array.from(e.target.files || []))} />
                <input className="rounded-md border px-3 py-2" placeholder="Describe your vision (e.g., 'make it more vibrant', 'remove the background')" value={url} onChange={(e) => setUrl(e.target.value)} />
                <button onClick={trainProfile} className="rounded-md bg-zinc-900 px-4 py-2 text-white">Process Image</button>
              </div>
              {result && (
            ...
            {mode === "scratch" && (
              <div className="rounded-md border bg-white p-4 text-sm text-zinc-600">Next: AI will enhance and style your image based on your description.</div>
            )}
            {mode === "improve" && (
              <div className="rounded-md border bg-white p-4 text-sm text-zinc-600">Next: AI will analyze and transform your image with advanced editing techniques.</div>
            )}
          </div>
        )}
      </div>
